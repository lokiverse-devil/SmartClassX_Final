                ...
                filteredRecords.map((record) => {
                  ...
                  return (
                    <tr key={record.id} className="hover:bg-muted/30">
                      <td className="px-6 py-4 text-sm">{format(new Date(record.date), 'PPP')}</td>
                      ...
                      <td className="px-6 py-4 text-sm">
                        {record.checkIn ? format(new Date(record.checkIn), 'pp') : <span className="text-muted-foreground">Not marked</span>}
                      </td>
                      <td className="px-6 py-4 text-sm">
                        {record.checkOut ? format(new Date(record.checkOut), 'pp') : <span className="text-muted-foreground">Not marked</span>}
                      </td>
                      <td className="px-6 py-4">
              ...
              {filteredRecords.map((record) => {
                ...
                return (
                  <div key={record.id} className="p-4 border rounded-lg bg-background shadow-sm">
                    <p><span className="font-semibold">Date:</span> {format(new Date(record.date), 'PPP')}</p>
                    <p><span className="font-semibold">Student:</span> {student?.name || 'Unknown'} (ID: {record.studentId})</p>
                    <p><span className="font-semibold">Department:</span> {student?.department || 'N/A'}</p>
                    <p><span className="font-semibold">Check-in:</span> {record.checkIn ? format(new Date(record.checkIn), 'pp') : 'Not marked'}</p>
                    <p><span className="font-semibold">Check-out:</span> {record.checkOut ? format(new Date(record.checkOut), 'pp') : 'Not marked'}</p>
                    <p>
